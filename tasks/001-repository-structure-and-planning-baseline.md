...
- `tasks/README.md` links to every numbered task file.
...
- Run `rg -n "PLAN_BI_TUTORIAL_APP|PLAN_BI_TUTORIAL_TUTORIALS|tasks/README" PLAN.md`.
- Run `rg --files tasks | sort`.
- Run `rg -L "^## Tests" tasks/*.md` and confirm no task file is missing a tests section.
- Run `rg -L "^## Verification" tasks/*.md` and confirm no task file is missing a verification section.
- Run `rg -n "Before starting any task|After finishing or pausing any task|BUGS.md" AGENTS.md tasks/README.md`.

## Verification Notes

- `PLAN.md` is an umbrella document and links the app plan, tutorial plan, and task index.
- `tasks/README.md` links every numbered task file from 001 through 018.
- Each numbered task file includes `Verification` and `Tests` sections.
- `AGENTS.md` and `tasks/README.md` require continuity-file review and updates before/after tasks.
