...
function stripConfiguredBase(value: string): string {
  if (configuredBase === undefined || configuredBase.length === 0 || configuredBase === './') {
    return value;
  }
...
function localReferencePath(value: string): string | undefined {
  ...
  const cleanedValue = stripConfiguredBase(stripQueryAndHash(value)).replace(/^\/+/u, '');
...
async function assertExists(path: string, reference: HtmlReference): Promise<void> {
  try {
    await access(path);
  } catch (error: unknown) {
    if (error instanceof Error && 'code' in error && error.code === 'ENOENT') {
      throw new Error(`${reference.attribute}="${reference.value}" points to missing built file ${path}.`, {
        cause: error,
      });
    }
