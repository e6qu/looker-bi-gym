...
function unwrapParse<T>(
  result: { readonly ok: true; readonly value: T } | { readonly ok: false; readonly message: string },
): T {
  if (!result.ok) {
...
assert.deepEqual(csvParse.columns, ['business_date', 'currency_code', 'ledger_total']);
...
assert.deepEqual(unwrapParse(jsonParse).columns, ['business_date', 'currency_code', 'ledger_total']);

const validEvaluation = evaluateCloudEvidenceChecks(challenge, validAnswers);
assert.equal(validEvaluation.requiredPassed, true);
assert.equal(validEvaluation.checks.every((check) => check.status === 'pass'), true);
