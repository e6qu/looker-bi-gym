...
assert.equal(
  evaluateQuestion(multipleChoiceQuestion, "docker").isCorrect,
  false,
);
assert.equal(
  evaluateQuestion(selectAllQuestion, ["synthetic_iban", "account_id"])
    .isCorrect,
  true,
);
assert.equal(
  evaluateQuestion(selectAllQuestion, ["account_id"]).isCorrect,
  false,
);
