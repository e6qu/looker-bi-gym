...
function parseRowCountExpectation(value: unknown): RowCountExpectation | undefined {
  ...
  if (
    (operator === 'equals' || operator === 'at-least' || operator === 'at-most') &&
...
function parseAggregateExpectation(value: unknown): AggregateExpectation | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function getSensitiveFields(challenge: ChallengeManifest, check: ChallengeCheck): readonly string[] {
  const expectedFields = parseStringList(check.expected) ?? [];
  const inputFields = challenge.inputs.flatMap((input) => input.sensitive_fields ?? []);

  return [...new Set([...inputFields, ...expectedFields])];
}

function hasRowCountPassed(actual: number, expected: RowCountExpectation): boolean {
...
function getNumericCell(row: Readonly<Record<string, unknown>>, column: string): number | undefined {
  const normalizedColumn = normalizeColumnName(column);
  const matchingKey = Object.keys(row).find((key) => normalizeColumnName(key) === normalizedColumn);
...
function evaluateRequiredColumns(
  ...
  if (expectedColumns === undefined) {
    return makeResult(check, 'fail', 'Required-column check expected must be a string or string array.');
  }
  ...
  return missingColumns.length === 0
    ? makeResult(check, 'pass', `Found required columns: ${expectedColumns.join(', ')}.`)
    : makeResult(check, 'fail', `Missing required columns: ${missingColumns.join(', ')}.`);
}
...
function evaluateForbiddenColumns(
  ...
  if (expectedColumns === undefined) {
    return makeResult(check, 'fail', 'Forbidden-column check expected must be a string or string array.');
  }

  const resultColumns = getResultColumns(result);
  const presentColumns = expectedColumns.filter(
    (column) => resultColumns.has(normalizeColumnName(column)),
  );

  return presentColumns.length === 0
    ? makeResult(check, 'pass', `Did not expose forbidden columns: ${expectedColumns.join(', ')}.`)
    : makeResult(check, 'fail', `Result exposes forbidden columns: ${presentColumns.join(', ')}.`);
}

function evaluateRowCount(check: ChallengeCheck, result: SqlValidationResult): ValidationCheckResult {
...
function evaluateUniqueKey(check: ChallengeCheck, result: SqlValidationResult): ValidationCheckResult {
  const keyColumns = parseStringList(check.expected);

  if (keyColumns === undefined || keyColumns.length === 0) {
    return makeResult(check, 'fail', 'Unique-key check expected must be a string or string array.');
  }
  ...
  if (missingColumns.length > 0) {
    return makeResult(check, 'fail', `Unique-key columns are missing: ${missingColumns.join(', ')}.`);
  }
  ...
  for (const row of result.rows) {
    ...
    if (seenKeys.has(key)) {
      return makeResult(check, 'fail', `Duplicate key found for ${keyColumns.join(', ')}.`);
    }

    seenKeys.add(key);
  }

  return makeResult(check, 'pass', `Rows are unique by ${keyColumns.join(', ')}.`);
}
...
function evaluateScalarAggregate(
  ...
  if (result.rows.length !== 1) {
    return makeResult(check, 'fail', `Expected one aggregate row, got ${result.rows.length}.`);
  }
  ...
  if (actualValue === undefined) {
    return makeResult(check, 'fail', `Aggregate column ${expectation.column} is missing or not numeric.`);
  }

  const delta = Math.abs(actualValue - expectation.value);

  return delta <= expectation.tolerance
    ? makeResult(check, 'pass', `${expectation.column} matched expected value ${expectation.value}.`)
    : makeResult(
        check,
        'fail',
...
export function evaluateSqlResultCheck(
  ...
  switch (check.type) {
    ...
    case 'sensitive-field-exclusion':
      return evaluateForbiddenColumns(check, result, getSensitiveFields(challenge, check));
    default:
      return makeResult(check, 'unsupported', `Unsupported browser SQL validator: ${check.type}.`);
  }
}
